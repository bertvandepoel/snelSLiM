...
func main() {
	...
	var xpath string
	cmd_sed := exec.Command("/bin/sed", "-e", "s/xmlns=/ignore=/", filename)
	if lemma {
		xpath = "//w/@lemma|//w/@lem"
	} else {
		xpath = "//w"
	}
	cmd_xmllint := exec.Command("/usr/bin/xmllint", "--xpath", xpath, "-")
	cmd_xmllint.Stdin, _ = cmd_sed.StdoutPipe()
	output := new(bytes.Buffer)
	cmd_xmllint.Stdout = output
	err := cmd_xmllint.Start()
	if err != nil {
	...
	err = cmd_sed.Run()
	if err != nil {
		fmt.Println("There was a problem executing sed")
		panic(err)
	}
	err = cmd_xmllint.Wait()
	if err != nil {
		fmt.Println("There was a problem executing xmllint")
		panic(err)
	}
